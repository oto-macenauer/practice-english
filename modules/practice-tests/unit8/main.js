(async () => {
  ...
  const SECTION_META = {
    "going-to":              { icon: "🔮", color: "blue" },
    "directions":            { icon: "🧭", color: "teal" },
    "prepositions-movement": { icon: "➡️", color: "purple" },
    "places-in-town":        { icon: "🏙️", color: "rose" },
    "town-vs-city":          { icon: "🏘️", color: "amber" },
    "emotions":              { icon: "😊", color: "orange" },
    listening:               { icon: "🎧", color: "sky" },
    "city-break":            { icon: "🗽", color: "indigo" },
    "road-to-hope":          { icon: "🛣️", color: "emerald" },
  };
  ...
  activeSections.forEach((section) => {
    ...
    checkBtn.addEventListener("click", () => {
      ...
      App.saveScore("practice-tests", "unit8", section.id, sectionScore, items.length);
  ...
  function showGrade(correct, total) {
    ...
    App.saveScore("practice-tests", "unit8", "_total", correct, total, grade);
  }

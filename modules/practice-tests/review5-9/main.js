(async () => {
  ...
  const SECTION_META = {
    "grammar-mix":    { icon: "📚", color: "blue" },
    "vocab-mix":      { icon: "🔤", color: "rose" },
    listening:        { icon: "🎧", color: "sky" },
    "review-reading": { icon: "📖", color: "emerald" },
  };

  const READING_PICK = 1; // number of reading sections per test
  ...
  activeSections.forEach((section) => {
    ...
    checkBtn.addEventListener("click", () => {
      ...
      App.saveScore("practice-tests", "review5-9", section.id, sectionScore, items.length);
  ...
  function showGrade(correct, total) {
    ...
    App.saveScore("practice-tests", "review5-9", "_total", correct, total, grade);
  }

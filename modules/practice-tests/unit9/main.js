(async () => {
  ...
  const SECTION_META = {
    ...
    "before-after-when":   { icon: "⏰", color: "blue" },
    "ed-ing-adjectives":   { icon: "🎭", color: "purple" },
    "travel-vocab":        { icon: "🧳", color: "rose" },
    "popocatepetl":        { icon: "🌋", color: "amber" },
    "compass-directions":  { icon: "🧭", color: "teal" },
    listening:             { icon: "🎧", color: "sky" },
    "summer-camp":         { icon: "✉️", color: "indigo" },
    "popocatepetl-story":  { icon: "🏔️", color: "emerald" },
  };
  ...
  activeSections.forEach((section) => {
    ...
    checkBtn.addEventListener("click", () => {
      ...
      App.saveScore("practice-tests", "unit9", section.id, sectionScore, items.length);
  ...
  function showGrade(correct, total) {
    ...
    App.saveScore("practice-tests", "unit9", "_total", correct, total, grade);
  }
